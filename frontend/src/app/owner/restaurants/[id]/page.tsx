...
export default function OwnerRestaurantManagePage() {
  ...
  useEffect(() => {
    if (restaurantId === "new") {
      // For new restaurant, initialize empty form
      setLoading(false);
      setFormData({
        name: "",
        description: "",
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      if (restaurantId === "new") {
        // TODO: Implement create restaurant endpoint
        alert("Create restaurant functionality coming soon!");
        // After creation, redirect to the new restaurant's page
        // router.push(`/owner/restaurants/${newRestaurantId}`);
      } else {
      ...
      alert(err.response?.data?.detail || "Failed to save restaurant");
    } finally {
      setSaving(false);
    }
  };
  ...
  // Show error only if it's not a new restaurant and there's an error
  if (restaurantId !== "new" && (error || !restaurant)) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-pink-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              {restaurantId !== "new" && (
                <button
                  onClick={() =>
                    router.push(`/owner/restaurants/${restaurantId}/menu`)
                  }
